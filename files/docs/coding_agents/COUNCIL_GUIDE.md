...
> **Purpose**: Operational guide for running multi-agent councils with cross-model deliberation. Councils use Claude teammates (Opus) for structured debate, plus Codex (OpenAI), Gemini (Google), and Cursor (agent) as independent external analysts.
...
Both run in background via CLI — launched with `run_in_background: true` in the Bash tool.
...
- Launch both at the same time as Claude teammates (Phase 1)
- Use `stream-json` output format for both
...
Give both Codex and Gemini the same prompt (adjusted only for the CLI syntax):
...
DELIVERABLE: Write your analysis to council/external-[codex|gemini]/NOTES.md
...
1. Launch Codex AND Gemini in background (full autonomy, no role, no coordination)
...
1. When Codex and Gemini complete, share BOTH sets of findings with all Claude teammates
2. Present them as "External Agent A" and "External Agent B" — let members react to each
3. Key questions to surface:
   - Did either external agent catch something no Claude member did?
   - Do Codex and Gemini agree with each other? Disagreements between them are high-signal.
   - Does either external analysis change a Claude member's position?
...
2. Note where Codex and Gemini **converged** (high confidence) vs **diverged** (needs attention)
...
| **Ignoring Codex/Gemini disagreements** | When two independent external models disagree, that's the highest-signal finding. Always surface and resolve. |
| **Contaminating external agents** | Don't share Claude members' work with Codex/Gemini. Their value is independent perspective from a different model family. |
...
# See also: GEMINI_CLI_SUBAGENT.md for full Gemini reference
# See also: SUBAGENT_GUIDE.md for general subagent patterns
```
