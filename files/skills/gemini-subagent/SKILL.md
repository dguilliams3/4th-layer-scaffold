...
description: Use when spawning Gemini CLI as a subagent for investigation, review, or council work — includes headless invocation, model selection, auth setup, output formats, and do/don't patterns.
---
...
## Comparison: All Four Subagent CLIs

| Capability | `gemini` | `agent` (Cursor) | `codex` (OpenAI) | Claude Code `Agent` tool |
|------------|----------|-------------------|-------------------|--------------------------|
| Headless flag | `-p "prompt"` | `--print "prompt"` | stdin via `-` | N/A (native subagent) |
| Auto-accept | `--yolo` | `--force --trust` | `--dangerously-bypass-approvals-and-sandbox` | `mode: "bypassPermissions"` |
| Output format | `--output-format stream-json` | `--output-format stream-json` | `--output-format stream-json` | Returns message directly |
| Model select | `-m gemini-3.1-pro-preview` | `--model composer-2-fast` | `--model gpt-5.3-codex` | `model: "opus"` / `"sonnet"` |
...
| Workspace | CWD + `--include-directories` | CWD | CWD (put path in prompt, not `-C`) | CWD (inherits parent) |
